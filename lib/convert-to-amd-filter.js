...
module.exports = class ConvertToAMD extends Filter {
  ...
  _processJsFile(code, relativePath) {
    ...
    const modifiedSource = replaceRequireAndDefine(
      ...
      relativePath.includes("vendor.js") || relativePath.includes("test-support.js")
    );
  ...
  async build() {
    ...
    indexFiles.forEach((indexFile) => {
      ...
      if (this.inline) {
        ...
        amdScripts.push(
          `<script src="${this.rootURL}${this.loadingFilePath}/vendor.js"></script><script src="${this.rootURL}${this.indexHtmlCaches[indexFile].loadingScript}" data-amd-loading=true></script>`
        );
        fs.writeFileSync(
          path.join(
            this.outputPath,
